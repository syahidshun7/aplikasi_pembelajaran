...
const form = useForm({
    ...
    file: null,
    // Method spoofing: Laravel butuh POST + _method PUT untuk upload file saat update
        ...
        <div class="min-h-screen p-2 md:p-12 font-['Press_Start_2P'] text-[#4ed4d4] flex justify-center items-start">
            ...
            <div
                ...
                <div class="p-4 md:p-8 border-b-4 border-slate-700 bg-slate-900/50">
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
                        <Link :href="route('lobby')"
                            class="text-slate-500 hover:text-white text-[12px] underline uppercase">
                            [ BACK_TO_LOBBY ]
                        </Link>
                ...
                <div class="p-4 md:p-8 space-y-6 md:space-y-8">
                    ...
                    <div class="space-y-4">
                        <h3 class="text-[12px] text-cyan-400 uppercase tracking-widest underline italic">
                            Mission_Objectives:</h3>
                        <div
                            class="bg-black/30 p-4 md:p-6 border-l-4 border-slate-700 leading-relaxed font-sans text-[14px] text-slate-300 whitespace-pre-wrap uppercase">
                            {{ quest.description || 'NO ADDITIONAL DATA PROVIDED BY THE GUILD.' }}
                        </div>
                    </div>

                    <div v-if="quest.status !== 'Done' && quest.status !== 'Completed'"
                        class="mt-8 p-4 md:p-6 border-2 border-dashed border-cyan-900 bg-black/20">
                        <h3 class="text-[12px] mb-6 uppercase tracking-widest"
                            :class="props.hasSubmitted ? 'text-yellow-500' : 'text-white'">
                        ...
                        <form @submit.prevent="submitReport" class="space-y-6">
                            <div>
                                <label
                                    class="block text-[12px] text-slate-500 mb-2 uppercase">Proof_of_Completion:</label>
                            ...
                            <div>
                                <label
                                    class="block text-[12px] text-slate-500 mb-2 uppercase">Evidence_Artifact:</label>
                                ...
                                <div v-if="props.hasSubmitted && props.existingSubmission?.file_path"
                                    ...
                                    <div class="flex flex-col gap-2">
                                        ...
                                        <a :href="'/storage/' + props.existingSubmission.file_path" target="_blank"
                                ...
                                <div v-if="form.errors.file" class="text-red-500 text-[12px] mt-2 uppercase">{{
                                    form.errors.file }}</div>
                            </div>
                            ...
                            <button type="submit" :disabled="form.processing"
                                ...
                                {{ form.processing ? 'TRANSMITTING...' : (props.hasSubmitted ? 'UPDATE_REPORT' :
                                'EXECUTE_REPORT') }}
                            </button>
                        </form>
                    </div>
